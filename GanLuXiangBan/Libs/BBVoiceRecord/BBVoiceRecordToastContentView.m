...
- (void)setUp
{
    ...
    [_imgRecord mas_updateConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake(self.imgRecord.image.size.width, self.imgRecord.image.size.height));
    }];
    
    CGSize powerSize = CGSizeMake(18, 56);
    [_powerView mas_updateConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(self.imgRecord);
        make.leading.equalTo(self.imgRecord.mas_trailing).offset(4);
...
- (void)setUp
{
    ...
    [_imgRelease mas_updateConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake(self.imgRelease.image.size.width, self.imgRelease.image.size.height));
    }];
...
- (void)showWithMessage:(NSString *)msg
{
    ...
    [_imgIcon mas_updateConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerX.equalTo(self);
        make.size.mas_equalTo(CGSizeMake(self.imgIcon.image.size.width, self.imgIcon.image.size.height));
    }];
    CGSize textSize = [_lbContent sizeThatFits:CGSizeZero];
    [_lbContent mas_updateConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(self).offset(-12);
